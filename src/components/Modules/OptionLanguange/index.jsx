...
const OptionLanguange = () => {
  const [, setLanguange] = useAtom(languangeAtom)

  const handleSelect = (event) => {
    setLanguange(event.target.value)
  }
  return (
    <div className="relative bg-[#1A1510] text-white rounded border border-gray-400 w-[200px] pl-8">
      ...
      <select className="w-full bg-transparent py-1" onClick={handleSelect}>
        <EachUtils
          of={LIST_LANGUANGE}
          render={(item, index) => (
            <option className="bg-[#1A1510]" key={index} value={item.value}>
              {item.name}
            </option>
          )}
        />
...
export default OptionLanguange
